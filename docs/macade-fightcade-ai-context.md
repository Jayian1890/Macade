...
| Unsupported | local replay playback, synctest, unproven `quark:training` challenge route behavior, full training Lua/movie replay subsystems, complete events/stats/timesync. These remain blocked rather than faked until the local replay backend, synctest backend migration, Lua training assets, or runtime evidence are available. |
