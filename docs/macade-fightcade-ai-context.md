...
| training | Web emits `fcade://training/{emulator}/{gameid}`; Macade maps this to native `macade:training,{gameid}`. Training challenges may use `quark:training,{gameid},{quarkid}.{playerid},{port},{delay}`. | Solo training is local and must not invent match-server metadata. |
