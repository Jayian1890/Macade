...
| pcap.out_of_sync | `/Users/jayian/Downloads/out of sync.pcapng` | Native served `sfiii3nr1` capture where the match stayed connected but opponent inputs appeared dead. Direct UDP and TCP startup succeeded; peer sent UDP type `3` input frames with `inputBytes=4`, while Macade sent `inputBytes=10` because DIP-switch bytes were appended to each per-frame input block. |
| pcap.summary | local `.build/ggponet-analysis/` output | Tracked analyzer tooling is restored at `Scripts/analyze-ggponet-capture.py`; generated summaries remain local/untracked because captures can contain IPs, quarks, and chat/control text. |
...
| UDP gameplay | Type `3` input send/receive with ack-based pending-output pruning; type `1/2`, `4/5` probes/replies. Per-frame input blocks include player controls plus allowed common inputs and analog values, but do not serialize DIP-switch bytes; DIP state remains local/static emulator state. Remote-player analog decode preserves Fightcade's per-player common-input slot advance even though common inputs are ignored for remote players. |
| Prediction/rollback | Bounded last-input prediction, saved-state history, rollback replay via `advance_frame`; rollback replay advances FBNeo's `nCurrentFrame` before synchronized input and leaves `nFramesEmulated` unchanged, matching Fightcade Win32 callback behavior. Native `ggpo_synchronize_input` calls now use a two-player count like `ggpofba-ng.exe` `FUN_004722f0`, not a four-player count. |
| TCP ongoing | Command `15` chat/control send, incoming `-8` chat/control event emission, `17` frame batches, `18` snapshots, and command `19` ranked replay upload. Embedded chat input now flows Swift key capture -> Unix datagram `chatBegin`/`chatUpdate`/`chatSubmit`/`chatCancel` -> native overlay input state -> `ggpo_client_chat`/TCP command `15`, passing submitted user text unchanged. Binary evidence: `ggpofba-ng.exe` `0045d960` calls `ggpo_client_chat(DAT_00b1a804, &DAT_00b19e08)` with the edit buffer unchanged, and `ggponet.dll` `10026380` serializes command `15` as `string quark`, `string text`. After successful local command `15` send, Macade immediately emits the submitted text to the visible overlay with the local side's matchinfo name, because the server does not reliably return a user-visible echo for the sender. Incoming `-8` records whose sender field is exactly `Command` are Fightcade control echoes and are filtered from the visible overlay; normal `-8` user chat still populates shared-memory overlay chat lines. |
...
| --- | --- | --- |
| Analyzer | `Scripts/analyze-ggponet-capture.py` is a self-contained pcapng analyzer for IPv4 TCP/UDP Fightcade captures. It decodes TCP client commands and server records, command `15` text, command `17` frame count/input size, UDP ASCII rendezvous/useports records, and UDP type `1/2/3/4/5` counts including type `3` start/ack/bitcount/input size. Validation against `/Users/jayian/Downloads/fight.pcapng` reproduced 5281 packets, 1266 UDP payload packets, UDP type counts `1:14, 2:13, 3:981, 4:76, 5:73`, no `useports`, startup commands `0/11/12/15 V14/17/15 C2`, command `17` size `18`, and bidirectional UDP type `3` size `18`. It also decoded paired stream captures `/Users/jayian/Downloads/fightcade.pcapng` and `/Users/jayian/Downloads/macade.pcapng` as client commands `0/20/12`, and the old native out-of-sync capture as TCP command `17` size `10` with UDP type `3` sizes `4` and `10`. | Use this analyzer for fresh post-fix capture checks before further packet-byte changes. It is not yet a full Phase 1 analyzer: it does not group attempts by quark/session, measure cadence precisely, or decode UDP type `3` bitstreams into per-frame values. |
...
| UDP type mix | Successful peer flow in the current pcap: type `3` count 981, type `4` count 76, type `5` count 73, type `1` count 10, type `2` count 10, token packets 6. Successful type `3` traffic proceeds bidirectionally after token exchange; the first observed type `3` input packets start at frame `2` with ack `-1` and input size `18`, and later packets can keep that cumulative base while carrying newer frames. Native UDP must accept initial/cumulative remote batches whose start frame is at or before the next required frame, and reject only true future gaps. A served-match stall on `1785449421959-5392.0` reached UDP punch and TCP startup, then published only the initial predicted frames; the cause was `MacadeCaptureLocalControls` returning one byte past the real per-player input block (`j + 1`), which could emit `19` bytes for sfiii3 instead of the observed official `18`. The fix is to return `j`, the one-past-end byte count. Quality report/reply cadence remains about 1000 ms. | Use these counts/cadences as initial smoke thresholds, not exact constants. |
| Out-of-sync native capture | `/Users/jayian/Downloads/out of sync.pcapng` for `sfiii3nr1` match `1785450749513-4202.0` shows direct UDP token punch and TCP startup completing, then ongoing UDP type `3` traffic. Peer packets are all `inputBytes=4`; Macade packets are all `inputBytes=10`, matching the local capture path appending six DIP bytes after the four-byte player/common control block. This explains isolated local play: both sides can remain connected while their input stream schemas diverge. | Do not serialize DIP-switch bytes in GGPO input frames. Validate the next live `sfiii3nr1` capture sends `inputBytes=4` from Macade and that remote inputs affect both clients. |
| Latest native out-of-sync follow-up | User reported the match remained out of sync after DIP removal. Local run `1785451462146-5619.1` used the fixed runtime but the log showed UDP punch completing with peer `192.168.1.254:6006` after the master returned public peer `172.7.214.252:32977`. A stricter source-validation attempt then caused black-screen/startup failure on run `1785452377000-5214.1`: the runtime rejected repeated `192.168.1.254` punch replies, failed all punch fallbacks, and exited before ROM/run-loop startup. | Do not reject those gateway/private punch replies without official evidence; the source-validation attempt was removed. Continue investigating out-of-sync through input/UDP parity and fresh captures rather than blocking startup. |
| Delayed TCP ready | Fresh logs from `1785453306969-5881.1` and `1785453376735-1760.1` showed player-1 attempts reaching TCP matchinfo then immediately disconnecting before ROM/load/frame publication, while Swift video diagnostics showed `no-snapshot`. Existing pcap decode showed official command `17` frame batches using the real queued input size (`60,18` in `/Users/jayian/Downloads/fight.pcapng`), while Macade's out-of-sync capture sent `60,10` from a hardcoded pre-driver startup batch. | Served TCP startup now sends only commands `0/11/12/15 V14` before driver init; the first command `17` batch and `15 C2,<player>,<delay>,<ranked>` are sent after `ggpo_synchronize_input` establishes the runtime input size. Live-validate that the remote no longer black-screens and capture the post-fix command `17` size. |
| TCP ready crash fix | Runs `1785454298396-3043.0` and `1785459423046-2206.0` reached UDP punch, TCP connect, ROM load, and `RunMessageLoop`, then crashed locally before first video snapshot. Crash reports `macfbneo-2026-07-30-183147.ips` and `macfbneo-2026-07-30-195711.ips` both faulted in `MacadeSendTCPReadyIfNeeded(GGPOSession*) + 352` at near-null addresses. The first fix removed invalid vector slicing, but follow-up runs `1785460233713-9794.1` and `1785460245102-6186.1` still faulted in `MacadeSendTCPReadyIfNeeded(GGPOSession*) + 288` while reading `session->localInputs` for startup command `17` history. | Startup command `17` now avoids `localInputs` entirely and emits 60 zero frames using the real initialized `session->inputSize`; recurring command `17` still uses local history after the session is running. Live-validate again with the post-`f22e8a46...` runtime before judging delayed TCP-ready behavior. |
| Native runtime | Current pcap decodes one startup command `17` frame batch before `C2`; native runtime preserves that order but delays those two records until after the first synchronized-input call so the command `17` input size comes from the initialized game instead of the stale hardcoded `10`. Native served startup is currently evidence-gated to the proven direct punch path, including restricted/fixed-port UDP retry: it does not send `useports/<quark>` or continue into open-port fallback after peer timeout or failed token punch. Future fallback handling requires a capture proving gameplay. SDL netplay run-loop parity was tightened after lagging matches reached `RunMessageLoop`: frame counters now advance before network input sync and are not rolled back if GGPO waits for missing remote input, rollback replay advances `nCurrentFrame` before synchronized input without increasing `nFramesEmulated`, active embedded matches use SDL audio-buffer fill pacing without an extra post-audio blocking `ggpo_idle(1)`, embedded spectator playback uses timer-driven pacing with timer-generated audio committed into SDL's playback ring, the fallback clock is milliseconds rather than microseconds, active-play `ggpo_idle` no longer blocks once per transport, embedded input skips SDL event pumping, and `DrvInit` is restored to the public Fightcade SDL structure without Macade network setup or net speed tweaks. Native runtime accepts local `macade:training` for solo training; current Windows parser evidence does not recognize `quark:training`, so Macade treats that legacy path as unproven, sends real command `15` chat/control records from embedded chat input using unchanged user text, locally echoes successfully sent user chat to the visible overlay, filters incoming `-8` sender `Command` control echoes out of visible overlay chat, implements the rollback `log_game_state` diagnostic callback, loads detectors after savestate setup, restores stream flags after driver init, sends TCP command frames as official-style split writes (`u32_be length`, then sequence/command/payload), emits Swift spectator routes with the lobby-advertised stream port, defers stream startup command `0` to spectator idle instead of construction, sends stream `20`/`12` once after command `0` response, drives embedded spectator sessions through a dedicated loop that polls `ggpo_idle` until TCP `-12` is loaded before `RunIdle` frame execution, accepts initial and cumulative UDP type `3` batches that overlap already-decoded remote input using the DLL-style last-received baseline for skipped old frames, publishes native overlay state into the SwiftUI shared-memory overlay header, suppresses macOS `SIGPIPE` on TCP sockets, sends command `19` GGPOTV replay uploads for completed ranked FT sets, emits the exact per-player input block byte count rather than one extra byte, excludes DIP-switch bytes from GGPO input frames, uses two-player synchronization calls, and avoids local input history access while building the initial TCP command `17` ready batch. Swift video presentation now uses `MTKView`, expands 16-bit shared frames to BGRA8 before upload, reuses Metal textures and the RGB565 conversion buffer, and draws with nearest sampling. Latest verified bundled runtime SHA-256 is `f22e8a463aeb51025dd2126748146e83bd8901fcce7f95993efc8812f82a0457`. | Keep tests/tools aligned with analyzer output before relying on them for parity; live-validate open-port fallback gameplay, chat/control behavior, training route startup, command `19` acceptance, direct route, stream pacing, spectator overlay, and post-DIP-removal served inputs before closing the gap. |
...
Use the tracked capture analyzer before processing new packet captures:

```sh
Scripts/analyze-ggponet-capture.py capture.pcapng \
  --server-ip <ggpo-server-ip> \
  --peer-ip <peer-public-ip> \
  --out-dir <capture-summary-output-dir>
```

If `--server-ip` is unavailable, the analyzer can infer TCP client/server direction from Fightcade server ports `7000`, `7001`, and `7002`; provide the server IP when known for cleaner UDP direction labels.
...
1. Run `Scripts/analyze-ggponet-capture.py` on the next post-`f22e8a463aeb51025dd2126748146e83bd8901fcce7f95993efc8812f82a0457` live capture and verify command `17` no longer uses hardcoded size `10`, startup order is `0/11/12/15 V14/17/15 C2`, and Macade UDP type `3` input size matches the peer for the selected game.
2. Start Phase B behavior-preserving ownership splits in the direct vendored source: move exported `ggpo_*` shims, callback/event helpers, served rendezvous/TCP code, and UDP ownership into focused files without changing packet bytes.
3. Migrate rollback/input queues from public GGPO only after those ownership boundaries are split and the rollback harness remains green.
4. Validate UDP reliability with controlled official loss/delay captures before changing resend, interruption, or timesync behavior.
5. Implement TCP connect/retry/disconnect parity after static review and official retry/disconnect evidence.
6. Keep open-port fallback gameplay, `ggpo_get_stats`, command `19` live validation, direct, stream, replay playback, and synctest evidence-gated.
