...
- Non-spectator embedded match play initializes SDL media after a Fightcade savestate is loaded successfully, preserving the official Win32 launch sequence where command-line `QuarkInit` is followed by app-level `MediaInit()` before entering the run loop. Delayed spectator loads keep their existing `DrvInit`/`MediaInit`/`RunInit` restore path.
- Embedded match play uses the Win32 app-side frame accumulator and `nRunQuark` idle gate instead of SDL audio callbacks or SDL's legacy no-audio scheduler, so GGPO prediction-barrier backpressure returns to idle before the next frame attempt. SDL audio is still kept live: embedded match frames write into `nAudNextSound`, and the SDL audio buffer is checked after the accumulator step instead of allowing the audio callback to drive frames. Embedded spectate keeps its audio-driven force-draw path and publishes a redraw immediately after first streamed state restore; otherwise SDL can produce audio-only frames with `pBurnDraw = NULL` and leave the embedded video stream black.
- SDL match play now initializes and updates Fightcade detector `.inf` files from the bundled runtime, matching the Win32 app-side detector state used by `fbn_ggpo.cpp`. Non-spectator GGPO save-state headers pack detector state, scores, ranked flag, and start bytes into header words 3 and 4; spectator loads restore those fields before applying the state bytes.
